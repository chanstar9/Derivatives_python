...
class ELS:
    ...
    def FDM_pricing(self, N, eval_date, vols, corr, IRTS, q):
        ...
        for _date in tqdm(list(reversed(self.dates))):
            u_NKI = get_payoff(u=u_NKI, redemption_dates=self.redemption_dates, coupon=self.coupon, F=self.F, K=self.K,
                               initial_underlyings=self.initial_underlyings, _date=_date, a=a, b=b, c=c, h=h, dt=dt,
                               prices=prices, vols=vols, corr=corr, N=N)
            u_KI = get_payoff(u=u_KI, redemption_dates=self.redemption_dates, coupon=self.coupon, F=self.F, K=self.K,
                              initial_underlyings=self.initial_underlyings, _date=_date, a=a, b=b, c=c, h=h, dt=dt,
                              prices=prices, vols=vols, corr=corr, N=N)
