...
for j in tows:
    ...
    delta = pd.concat([delta, pd.DataFrame(lst, index=s)], axis=1)

delta.columns = tows

fig = plt.figure()
ax = fig.gca(projection='3d')
xnew, ynew = np.meshgrid(s, tows)
...
# gamma
gamma = pd.DataFrame(index=s)
for j in tows:
    lst = []
    for i in s:
        lst.append(bs_gamma(i, k, j, r, q, vols[0], position))
    gamma = pd.concat([gamma, pd.DataFrame(lst, index=s)], axis=1)
...
# theta
theta = pd.DataFrame(index=s)
for j in tows:
    lst = []
    for i in s:
        lst.append(bs_theta(i, k, j, r, q, vols[0], position, cp=-1))
    theta = pd.concat([theta, pd.DataFrame(lst, index=s)], axis=1)
